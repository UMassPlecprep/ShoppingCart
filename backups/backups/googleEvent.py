...
def findEvent(summary,start):
    ...
    result = service.events().list(calendarId=cal_id,timeMin=start,singleEvents=True, maxResults=2500).execute()
...
def addEvent(prof,room,date,demos,start,end,isAstroClass):
    ...
    if result != 'Not found':
        print "Updating"
        event = service.events().get(calendarId=cal_id,eventId=result).execute()
        summary = summary
        ...
        print event
        service.events().update(calendarId=cal_id,eventId=result,body=event).execute()
        print "Done"
# color id 11 = red
# color id 10 = green
